<template>
	<view>
		<view class="diy-navbar  custom-top" :style="[{height:CustomBar + 'px',position:isFixed?'fixed !important':'relative !important'}]">
			<view class="custom-title flex fixed" :style="style" :class="[bgImage!=''?'none-bg text-white bg-img':'',bgColor]">
				<view class="diy-navbar-tool"  :style="[{top:StatusBar + 'px'}]" v-if="isBack||isHome">
					<div class="diy-navbar-tool-inner" :class="isCapsule?'diy-navbar-tool-solid':''" :style="[{border:isCapsule?'1px solid '+color:'none'}]">
