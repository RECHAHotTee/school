		...
		<view class="diy-sku">
			<view class="diy-sku__body flex flex-direction-column">
				<view class="flex margin-bottom-md flex-direction-row">
					...
					<view class="flex  flex-sub justify-between flex-direction-column">
						<view class="text-bold  align-baseline flex-direction-row">
							<text  :style="{color:priceColor}" class="custom-text-md">¥</text>
							<text  :style="{color:priceColor}" class="custom-text-lg margin-right-xs">{{selectedObj?selectedObj.price:price}}</text>
							<text  :style="{color:remarkColor}" class="custom-text-md line-price" v-if="(selectedObj&&selectedObj.linePrice)||linePrice">¥{{selectedObj?selectedObj.linePrice:linePrice}}</text>
						</view>
				...
				<view v-if="isStep"  class="diy-sku-step flex justify-between">
					<text class="custom-text-md">数量</text>
				...
				<view  v-if="isAddCart||isBuyNow"  class="flex custom-col-24">
					<button @tap="addCart" v-if="isAddCart" :style="{background:addCartBackground,color:addCartColor}" class="custom-btn   radius flex-sub margin-xs">{{addCartText}}</button>
					<button @tap="buyNow"  v-if="isBuyNow" :style="{background:buyNowBackground,color:buyNowColor}" :class="buyNowBackground" class="custom-btn   radius flex-sub margin-xs">{{buyNowText}}</button>
				</view>
			</view>
		</view>
	</u-popup>
