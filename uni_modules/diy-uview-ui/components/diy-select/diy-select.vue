			...
			<view class="u-select">
				...
				<view class="u-select__body">
					 ...
					 <scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y">
					 	<!--单选-->
					 	<u-radio-group class="flex diy-select flex-wrap custom-col-24 flex-direction-column justify-between" wrapClass="flex-direction-column" :activeColor="activeColor" v-if="mode == 'radio'" :borderBottom="true" iconPlacement="right" placement="column"
					 		@change="radioChange" >
					 	...
					 	<!--多选-->
					 	<u-checkbox-group class="flex diy-select flex-wrap custom-col-24 flex-direction-column justify-between" wrapClass="flex-direction-column" :activeColor="activeColor" v-if="mode == 'checkbox'" :borderBottom="true" placement="column"
					 ...
					 <view class="bottons">
					 	<u-row>
					 		...
					 		<u-col customStyle="padding:0 20rpx 20rpx 10rpx" span="6">
					 			<u-button @click="confirm" type="primary" throttleTime="1000" :disabled="(JSON.stringify(radioData) === '{}') && (checkboxData.length === 0)">确认</u-button>
					 		</u-col>
					 	</u-row>
					 </view>
				</view>
				...
				handler(val) {
					if(val){
						this.ulist =  this.list.filter(item=>{
							return item[this.labelName].indexOf(this.keyword)>-1||item[this.valueName].indexOf(this.keyword)>-1
						})
					}else{
						this.ulist =  this.list
					}
			...
			init(){
				if(this.mode=='checkbox'){
					this.list.forEach(item=>{
						item.checked = this.defaultValue.includes(item[this.valueName])
					})
				}
			},
			checkboxChange(n) {
				this.checkboxData = this.list.filter(item=>{
					return n.includes(item[this.valueName])
				})
			},
			//选择列表项触发
			radioChange(n) {
				this.radioData = this.list.find(item=>{
					return item[this.valueName] == n
				})
			},
			//点击搜索触发
			search() {
